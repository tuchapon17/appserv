...
class Article extends MY_Controller
{
	...
	function edit()
	{
		...
		if($this->frm->run() == false)
		{
			$this->set_default_sess_orderby("article", array("field"=>"article_name","type"=>"ASC"));
			//pagination
			...
			$search_text=null;
			$search_text=$this->session->userdata("search_article");
			if($search_text!=null)
			{
	...
	{
		if($this->sess_orderby_article["type"]=="ASC") $img='<img width="9" src="'.base_url().'images/glyphicons_free/glyphicons/png/glyphicons_212_down_arrow.png"';
		else if($this->sess_orderby_article["type"]=="DESC") $img='<img width="9" src="'.base_url().'images/glyphicons_free/glyphicons/png/glyphicons_213_up_arrow.png"';
		...
		';
		$html.='</thead>
		<form method="post" action="'.base_url().'?d=manage&c=article&m=delete" id="form_del_article" autocomplete="off">';
		if(!empty($data))
		{
			foreach ($data AS $dt):
			//<td>'.$num_row.'</td>
	...
	{
		$load=$this->load_article_model->load_article($this->input->post("tid"));
