...
class Occupation extends MY_Controller
{
	...
	function add()
	{
		...
		else
		{
			...
			//add event
			$this->add_event($this->lang->line("ti_add_occupation"));
	...
	function edit()
	{
		...
		else
		{
			...
			//add event
			$this->add_event("แก้ไข".$this->lang->line("text_occupation"));
	...
	{
		//add event
		$this->add_event("ลบ".$this->lang->line("text_occupation"));
	...
	function table_edit($data)
	{
		...
		if(!empty($data))
		{
			...
			*/
			if($dt["checked"] == 0 ) $checked='<i id="checked'.$dt["occupation_id"].'" class="fa fa-circle fa-danger fa-lg status0" onclick=toggle_checked("'.$dt["occupation_id"].'")></i>';
			else $checked='<i id="checked'.$dt["occupation_id"].'" class="fa fa-circle fa-success fa-lg status1" onclick=toggle_checked("'.$dt["occupation_id"].'")></i>';
			$html.='<tr>
	...
	function toggle_checked()
	{
		...
		if($do == "enable")
		{
			...
			$where = array("occupation_id"=>$id);
			$this->db->update("tb_occupation",$set,$where,1);
