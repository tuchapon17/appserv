...
class Room_type_model extends MY_Model
{
	...
	function get_room_type_list($perpage,$getpage=0,$liketext='')
	{
		...
		if($liketext!='')
		{
			$query=$this->db->select()->from("tb_room_type")->like($searchfield,$liketext,"both")->order_by("CONVERT(".$orderby_filed." USING ".$this->mysql_charset.")",$orderby_type)->limit($perpage,$getpage)->get();
		}
		else
		{
			$query=$this->db->select()->from("tb_room_type")->order_by("CONVERT(".$orderby_filed." USING ".$this->mysql_charset.")",$orderby_type)->limit($perpage,$getpage)->get();
		}
		if($query->num_rows()>0)
		{
