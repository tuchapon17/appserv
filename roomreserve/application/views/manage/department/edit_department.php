...
$eml=$ci->element_lib;
$sess_orderby_department=$this->session->userdata("orderby_department");
      		...
      		<div class="col-lg-12" id="loginform">
      			...
      			<div class="panel panel-success">
					<div class="panel-heading">
						<h3 class="panel-title"><strong>แก้ไข<?php echo $this->lang->line("text_department");?></strong></h3>
					</div>
					<div class="panel-body">
						<form role="form" action="?d=manage&c=<?=$controller?>&m=edit" method="post" autocomplete="off" id="edit_department">
								<?php
								echo $in_department;
...
<!-- Custom Javascript -->
	...
	$(function(){
		$("#edit_department").validate({
			...
			rules: {
				"<?php echo $this->lang->line("in_department");?>": {
					...
					remote:{
						...
						url:b_url+"?d=manage&c=department&m=already_exist_ajax",
						type:"POST"
					}
				}
			},
			messages:{
				"<?php echo $this->lang->line("in_department");?>":{
					remote:"<?php echo $this->lang->line("t_in_department");?>นี้ถูกใช้แล้ว"
				}
			}
		});
	...
	function load_department(tid)
	{
		//show data in input
		$.ajax({
			url:"?d=manage&c=<?=$controller?>&m=load_department",
			data:"tid="+tid,
			type:"POST",
			dataType:"json",
	...
	{
		var select_field='<option value="department_id">รหัส<?php echo $this->lang->line("text_department");?></option>';
		select_field+='<option value="department_name"><?php echo $this->lang->line("text_department");?></option>';
		...
		var sess_f="<?php echo $sess_orderby_department["field"];?>";
		var sess_t="<?php echo $sess_orderby_department["type"];?>";
	...
	function toggle_checked(id)
	{
		...
		{
			$.ajax({
				url:"?d=manage&c=department&m=toggle_checked",
				data:{"id":id,"s":"enable"},
				type:"POST",
				dataType:"text",
				success:function(resp){
