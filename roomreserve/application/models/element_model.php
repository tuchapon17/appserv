...
class Element_model extends CI_Model
{
	...
	function select_province()
	{
		...
		$this->db->select()->from("tb_province")->order_by("CONVERT(province_name USING UTF8)","ASC");
		$query=$this->db->get();
		return $query->result_array();
	}
	function select_district($province_id)
	{
		$this->db->select()->from("tb_district")->like('district_id',$province_id,'after')->order_by("CONVERT(district_name USING UTF8)","ASC");
		$query=$this->db->get();
		return $query->result_array();
	}
	function select_subdistrict($district_id)
	{
		$this->db->select()->from("tb_subdistrict")->like('subdistrict_id',$district_id,'after')->order_by("CONVERT(subdistrict_name USING UTF8)","ASC");
		$query=$this->db->get();
		return $query->result_array();
	}
	function select_article_type()
	{
		$this->db->select()->from("tb_article_type")->order_by("CONVERT(article_type_name USING UTF8)","ASC");
		$query=$this->db->get();
		return $query->result_array();
	}
	...
	{
		$this->db->select()->from("tb_faculty")->where("checked",1)->order_by("CONVERT(faculty_name USING UTF8)","ASC");
		$query=$this->db->get();
		return $query->result_array();
	}
	...
	{
		if($where=='')
			$this->db->select()->from($table_name)->order_by("CONVERT(".$order_field." USING UTF8)","ASC");
		else $this->db->select()->from($table_name)->where($where)->order_by("CONVERT(".$order_field." USING UTF8)","ASC");
		$query=$this->db->get();
		return $query->result_array();
	}
	function select_room_list($room_type_id)
	{
		$this->db->select()->from("tb_room")->like('tb_room_type_id',$room_type_id,'both')->order_by("CONVERT(room_name USING UTF8)","ASC");
		$query=$this->db->get();
		return $query->result_array();
	}
	function select_room_rha($article_id)
	{
		...
		$query=$this->db->query("SELECT * FROM tb_room WHERE NOT EXISTS (SELECT * FROM tb_room_has_article WHERE  tb_room.room_id=tb_room_has_article.tb_room_id AND tb_room_has_article.tb_article_id='".$article_id."') ORDER BY CONVERT(room_name USING UTF8 ) ASC");
		//$query=$this->db->get();
		return $query->result_array();
	}
	function select_person_list($person_type_id)
	{
		$this->db->select()->from("tb_person")->like('tb_person_type_id',$person_type_id,'both')->order_by("CONVERT(person USING UTF8)","ASC");
		$query=$this->db->get();
		return $query->result_array();
	}
