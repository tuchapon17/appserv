...
class Occupation_model extends MY_Model
{
	...
	function get_occupation_list($perpage,$getpage=0,$liketext='')
	{
		...
		if($liketext!='')
		{
			$query=$this->db->select()->from("tb_occupation")->like($searchfield,$liketext,"both")->order_by("CONVERT(".$orderby_filed." USING UTF8)",$orderby_type)->limit($perpage,$getpage)->get();
		}
		else
		{
			$query=$this->db->select()->from("tb_occupation")->order_by("CONVERT(".$orderby_filed." USING UTF8)",$orderby_type)->limit($perpage,$getpage)->get();
		}
		if($query->num_rows()>0)
		{
