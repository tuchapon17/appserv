...
$eml=$ci->element_lib;
$sess_orderby_office=$this->session->userdata("orderby_office");
$controller="office";
$m_name="office";
echo $htmlopen;
echo $head;
...
<!-- Custom Javascript -->
	...
	function load_office(tid)
	{
		//show data in input
		$.ajax({
			url:"?d=manage&c=<?=$controller?>&m=load_office",
			data:"tid="+tid,
			type:"POST",
			dataType:"json",
			success:function(resp){
				$("#input_office").val(resp.office_name);
			},
	...
	{
		var select_field='<option value="office_id">รหัส หน่วยงาน</option>';
		...
		var sess_f="<?php echo $sess_orderby_office["field"];?>";
		var sess_t="<?php echo $sess_orderby_office["type"];?>";
	...
	function toggle_checked(id)
	{
		...
		{
			$.ajax({
				url:"?d=manage&c=office&m=toggle_checked",
				data:{"id":id,"s":"enable"},
				type:"POST",
				dataType:"text",
				success:function(resp){
