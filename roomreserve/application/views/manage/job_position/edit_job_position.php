...
$eml=$ci->element_lib;
$sess_orderby_job_position=$this->session->userdata("orderby_job_position");
      		...
      		<div class="col-lg-12" id="loginform">
      			...
      			<div class="panel panel-success">
					<div class="panel-heading">
						<h3 class="panel-title"><strong>แก้ไข<?php echo $this->lang->line("text_job_position");?></strong></h3>
					</div>
					<div class="panel-body">
						<form role="form" action="?d=manage&c=<?=$controller?>&m=edit" method="post" autocomplete="off" id="edit_job_position">
								<?php
...
<!-- Custom Javascript -->
	...
	function load_job_position(tid)
	{
		//show data in input
		$.ajax({
			url:"?d=manage&c=<?=$controller?>&m=load_job_position",
			data:"tid="+tid,
			type:"POST",
			dataType:"json",
	...
	{
		var select_field='<option value="job_position_id">รหัส<?php echo $this->lang->line("text_job_position");?></option>';
		select_field+='<option value="job_position_name"><?php echo $this->lang->line("text_job_position");?></option>';
		...
		var sess_f="<?php echo $sess_orderby_job_position["field"];?>";
		var sess_t="<?php echo $sess_orderby_job_position["type"];?>";
	...
	function toggle_checked(id)
	{
		...
		{
			$.ajax({
				url:"?d=manage&c=job_position&m=toggle_checked",
				data:{"id":id,"s":"enable"},
				type:"POST",
				dataType:"text",
				success:function(resp){
